...
local backend_select = require "backend-select"
...
M.LOCK_EXPTIME_SEC = 2                           -- Lock expiry to prevent stale locks
M.LOCKS_SHDICT = "token_bucket_locks"            -- Shared dict used for resty.lock state
...
--   set $refill_rate 500;
-- Optional scope for per-tier buckets:
...
function M.apply_rate_limit(client_ip, cost, is_exempt)
    ...
    local bucket_tier = ngx.var.rate_limit_tier
    ...
    local lock, err = resty_lock:new(M.LOCKS_SHDICT, {
        timeout = M.LOCK_TIMEOUT_SEC,
        exptime = M.LOCK_EXPTIME_SEC
    })
    if not lock then
    ...
    local elapsed, err = lock:lock("lock:" .. bucket_key)
    if not elapsed then
        return false, "busy", nil
    end
    ...
    if tokens == nil then
        ...
        -- Clamp to the current capacity in case configuration changed.
...
local function get_backend_addrs()
    local backend_cache = ngx.shared.backend_cache
    local leader_addr = backend_cache:get("leader")
    local follower_addr = backend_cache:get("follower")
    if not follower_addr or follower_addr == "" then
        follower_addr = backend_cache:get("follower_1")
    end

    return leader_addr, follower_addr
end

local function pick_backend_addr(use_leader)
    return backend_select.pick_backend(use_leader, ngx.shared.backend_cache)
end
...
function M.get_backend(ctx, use_leader)
    ...
    if use_leader then
        name = "leader"
        entry = ctx.backends.leader
    elseif ctx.backends.follower.addr and ctx.backends.follower.addr ~= "" then
        if ctx.is_single_backend then
            name = "leader"
            entry = ctx.backends.leader
        else
            name = "follower"
            entry = ctx.backends.follower
        end
    else
        name = "leader"
        entry = ctx.backends.leader
    ...
    if not entry or not entry.addr or entry.addr == "" then
        return nil, name
    end
...
local function handle_jsonrpc_message(ctx, data, typ)
    ...
    if not ok then
        if err_type == "busy" then
            ...
            send_to_client(ctx, "text", '{"jsonrpc":"2.0","error":{"code":-32000,"message":"Rate limit exceeded","data":{"cost":' .. cost .. ',"remaining":' .. math.floor(remaining or 0) .. ',"method":' .. cjson.encode(method) .. '}},"id":' .. id .. '}')
        end
        return true  -- continue
    end
...
function M.handle_websocket()
    ...
    local leader_addr, follower_addr = get_backend_addrs()
    if not leader_addr and not follower_addr then
        ngx.status = 503
    ...
    local ctx = {
        ...
        is_single_backend = (leader_addr and follower_addr and leader_addr == follower_addr),
...
function M.handle_http_request(uri, http_method)
    ...
    if not ok then
        ...
        if jsonrpc_method then
            ...
            if err_type == "busy" then
                ...
                ngx.say('{"jsonrpc":"2.0","error":{"code":-32000,"message":"Rate limit exceeded","data":{"cost":' .. cost .. ',"remaining":' .. math.floor(remaining or 0) .. ',"method":' .. cjson.encode(jsonrpc_method) .. '}},"id":' .. jsonrpc_id .. '}')
            end
            return ngx.exit(200)
        else
    ...
    ngx.var.backend = pick_backend_addr(use_leader)

    if not ngx.var.backend or ngx.var.backend == "" then
        if jsonrpc_method then
            ngx.say('{"jsonrpc":"2.0","error":{"code":-32603,"message":"Backend unavailable"},"id":' .. jsonrpc_id .. '}')
            return ngx.exit(200)
        end
        ngx.status = 503
        ngx.say('{"error":"backend unavailable"}')
